...
export default function AdminLayout({
  ...
  return (
    <div className="flex min-h-screen bg-[#050505] text-[#FAFAFA] selection:bg-amber-500/30">
      {/* Sidebar — Desktop */}
      <aside className="hidden lg:flex flex-col w-[260px] fixed inset-y-0 left-0 z-30 bg-[#050505] border-r border-white/5">
        ...
        <div className="h-20 flex items-center gap-3 px-8">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-gradient-to-br from-amber-400 to-amber-600 text-[#050505] text-xs font-black shadow-[0_0_15px_rgba(245,158,11,0.2)]">
            A
          </div>
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded-xl text-[13px] font-semibold transition-all min-h-[44px] ${
                  isActive
                    ? "text-white bg-white/5 shadow-[inset_0_1px_rgba(255,255,255,0.05)] border-l-[2px] border-amber-400"
                    : "text-[#A1A1AA] hover:text-[#FAFAFA] hover:bg-white/[0.02] border-l-[2px] border-transparent"
                }`}
              >
                <div className={isActive ? "text-amber-400" : "opacity-70"}>{item.icon}</div>
                {item.label}
              </Link>
            );
          })}
      ...
      <header className="lg:hidden fixed top-0 left-0 right-0 z-40 bg-[#050505]/80 backdrop-blur-2xl h-14 flex items-center justify-between px-4 border-b border-white/5">
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded flex items-center justify-center bg-gradient-to-br from-amber-400 to-amber-600 text-[#050505] text-[10px] font-black">
            A
          </div>
