...
/**
 * AuthChrome — toda parte interativa da shell autenticada (sidebar desktop,
 * header mobile, tab bar mobile). Recebe dados do usuário como props vindos
 * do `(auth)/layout.tsx` server component (que pega via `currentUser()`).
 *
 * Por que existir separado:
 *   - Permite que `layout.tsx` seja server component e o HTML chrome chegue
 *     no first byte da resposta — antes a layout inteira hidratava no client
 *     e a sidebar só aparecia depois do JS rodar.
 *   - Hooks que precisam de client (usePathname, useClerk, useStoreUsage)
 *     ficam isolados aqui sem forçar a layout pai a ser client também.
 */
