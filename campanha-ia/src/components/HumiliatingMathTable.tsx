...
export default function HumiliatingMathTable() {
  const rows = [
    {
    ...
    <div className="w-full max-w-4xl mx-auto py-8">
        ...
        <div className="flex flex-col">
          {rows.map((row, i) => (
            <div key={i} className="grid grid-cols-2 divide-x divide-gray-200 dark:divide-gray-800 border-t border-gray-200 dark:border-gray-800 text-sm md:text-base">
              ...
              <div className="p-4 md:p-6 text-center flex flex-col items-center justify-center bg-brand-50 dark:bg-brand-900/10">
                <span className="text-xs text-brand-400 dark:text-brand-300/70 font-medium uppercase tracking-wider mb-1">{row.feature}</span>
                <span className="font-bold text-brand-700 dark:text-brand-300">{row.crialook}</span>
              </div>
            </div>
          ))}

          {/* Total Row */}
          <div className="grid grid-cols-2 divide-x divide-gray-200 dark:divide-gray-800 border-t-2 border-gray-200 dark:border-gray-800 text-base md:text-xl">
            <div className="p-6 md:p-8 text-center bg-gray-100 dark:bg-gray-800/50">
              <span className="block text-sm text-gray-500 mb-2">Custo de 1 campanha</span>
              <span className="font-black text-gray-400 line-through">R$ 1.650+</span>
            </div>
            <div className="p-6 md:p-8 text-center bg-brand-600 relative overflow-hidden">
              <div className="absolute inset-0 bg-gradient-to-r from-brand-500 to-brand-700 opacity-80" />
      ...
      <div className="mt-8 text-center">
        <motion.div 
          ...
          viewport={{ once: true }}
          className="inline-flex items-center gap-4 bg-surface border border-border px-6 py-4 rounded-full shadow-lg"
        >
