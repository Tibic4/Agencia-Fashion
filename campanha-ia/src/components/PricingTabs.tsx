      ...
      <AnimatePresence mode="wait">
        ...
        >
          {(activeTab === "assinaturas" ? assinaturas : packs).map((plan, i) => (
            <div 
              key={i} 
              className={`relative flex flex-col rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-8 bg-surface border transition-all duration-300 ${plan.popular ? 'border-brand-500 shadow-[0_0_40px_rgba(217,70,239,0.15)] mt-6 md:mt-0 md:-translate-y-4 hover:-translate-y-6' : 'border-border shadow-md hover:shadow-xl hover:-translate-y-2'}`}
            >
              {plan.popular && (
              ...
              <div className="mb-2">
                <span className="text-2xl sm:text-3xl md:text-5xl font-black text-foreground">
                  <span className="text-base sm:text-lg md:text-2xl font-bold opacity-60 mr-1">R$</span>
                  {plan.price}
                </span>
